...
// but it find t-value more correct, triangle ID and UV coordinates.
...
asm
  {$R-}
  // Based on Valve VRAD Ray-Winding intersection, but SSE-accelerated
  {for i:=0 to (Poly.CountVertecies - 1) do
    begin
...
@@SidePlanesLoop:
  ...
  PMOVMSKB EBX, XMM0		// SSE2 only one here instruction
  //NOT      EBX
  TEST     EBX, EBX
  // end loop body
...
begin
  {$R-}
  if (Src = nil) or (Dst = nil) or (Count <= 0) then Exit;
  for i:=0 to (Count - 1) do
    begin
      ARGB888(Dst)[i].r:=GAMMA_LUT[ARGB888(Src)[i].r];
      ARGB888(Dst)[i].g:=GAMMA_LUT[ARGB888(Src)[i].g];
      ARGB888(Dst)[i].b:=GAMMA_LUT[ARGB888(Src)[i].b];
    end; //}
  {$R+}
end;
